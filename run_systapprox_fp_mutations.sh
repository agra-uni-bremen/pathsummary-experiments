...
mutations_systolic=() #1 2 3 4)
mutations_approx=(1 2 3 4)

# systolic
widths=(2)
heights=(2)
matrixsizes=(2) # 4 8) # 16 32 64
# approx
intervals=(1)
segments=(16)
iterations=(1) # 2)
  ...
  do
	...
	do
	  ...
	  do
            ...
            do
              for mutation in "${mutations_approx[@]}"
              do
                tests+=("sysc_syst_${width}_${height}_${size}_approx_fp_${interval}_${segment}_${iteration}_FAULT${mutation}#${funcnum}")
              done
              for mutation in "${mutations_systolic[@]}"
              do
                tests+=("sysc_syst_${width}_${height}_${size}_FAULT${mutation}_approx_fp_${interval}_${segment}_${iteration}#${funcnum}")
	      done
	    done
          done
        done
