...
../configure --quiet CC=clang CMAKE_CXX_STANDARD="17" CCFLAGS="" CXX=clang++ CXXFLAGS="-std=c++17 -stdlib=libc++" --prefix=$PREFIX --with-arch-suffix=-native --disable-async-updates --enable-pthreads
...
../configure --quiet --target=x86_64-pc-linux-gnu CC=clang CMAKE_CXX_STANDARD="17" CFLAGS='' CXX=clang++ CXXFLAGS='-std=c++17 -stdlib=libc++' --prefix=$PREFIX --with-arch-suffix=-$LIBNAME --disable-async-updates --enable-pthreads
cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DDISABLE_ASYNC_UPDATES=1 -DENABLE_PTHREADS=1 -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_CXX_STANDARD="17" -D CMAKE_CXX_FLAGS="-DUSE_KLEE -fcxx-exceptions -stdlib=libc++ -std=c++17 -emit-llvm -flto -c -Xclang -disable-O0-optnone" -D CMAKE_CXX_CREATE_STATIC_LIBRARY="llvm-link <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> -o <TARGET> <LINK_LIBRARIES>" -D CMAKE_CXX_LINK_EXECUTABLE="llvm-link <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> <LINK_LIBRARIES> -o <TARGET>" -DCMAKE_LINKER=llvm-link -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$PREFIX -DINSTALL_TO_LIB_TARGET_ARCH_DIR=$LIBNAME ..
...
cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DDISABLE_ASYNC_UPDATES=1 -DENABLE_PTHREADS=1 -DCMAKE_C_FLAGS="-emit-llvm" -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_STANDARD="17" -DCMAKE_CXX_COMPILER=clang++ -D CMAKE_CXX_FLAGS="-DUSE_KLEE -fcxx-exceptions -stdlib=libc++ -std=c++17 -emit-llvm -flto -c -Xclang -disable-O0-optnone" -D CMAKE_CXX_CREATE_STATIC_LIBRARY="llvm-link <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> -o <TARGET> <LINK_LIBRARIES>" -D CMAKE_CXX_LINK_EXECUTABLE="llvm-link <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> <LINK_LIBRARIES> -o <TARGET>" -DCMAKE_LINKER=llvm-link -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$PREFIX -DINSTALL_TO_LIB_TARGET_ARCH_DIR=$LIBNAME ..
...
#../configure --quiet CC=wllvm CXX=wllvm++ --prefix="$PREFIX" --build=x86_64-pc-linux-gnu --enable-debug CXXFLAGS='-std=c++17 -stdlib=libc++' --with-arch-suffix=-wllvm --disable-async-updates --enable-pthreads #--enable-shared=NO --enable-static=YES
